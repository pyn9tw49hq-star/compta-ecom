...
interface CommissionPieChartProps {
  data: CommissionItem[];
  total: number;
}

/**
 * Donut chart showing commission HT distribution per channel.
 */
export default function CommissionPieChart({ data, total }: CommissionPieChartProps) {
  const reducedMotion = useReducedMotion();

  return (
    <ChartCard
      title="Répartition Commissions HT"
      subtitle="Part de chaque canal"
      minHeight={280}
      empty={data.length === 0}
      accessibleTable={{
        caption: "Répartition des commissions HT par canal",
        headers: ["Canal", "Commissions HT", "Part"],
        rows: data.map((d) => [
          d.label,
    ...
    >
      <ResponsiveContainer width="100%" height={280}>
        <PieChart role="img" aria-label="Commissions HT par canal de vente, diagramme en donut">
          <Pie
            data={data}
            dataKey="value"
            nameKey="label"
          ...
          <Tooltip
            content={({ active, payload }) => {
              ...
              const item = payload[0].payload as CommissionItem;
